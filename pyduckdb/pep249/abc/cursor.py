...
this is implemented as a set of mixins:
...
from .transactions import TransactionFreeContextMixin
...
class CursorSetSizeMixin(metaclass=ABCMeta):
    """ implementation of size setting for cursor."""
...
class CursorFetchMixin(metaclass=ABCMeta):
    ...
    def rowcount(self) -> int:
        ...
        execute call produced (for e.g. SELECT calls) or affected (for e.g.
        UPDATE/INSERT calls).
    ...
    def fetchmany(self, size: Optional[int] = None) -> ResultSet:
        ...
        produce a result set, an error will be raised.
